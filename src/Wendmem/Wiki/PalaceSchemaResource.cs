...
static class PalaceSchemaResource
{
        ...
        CancellationToken ct)
    {
        ...
        sb.AppendLine("- `WikiWrite(path, title, content, citations, wing?)` — create or update a wiki page.");
        ...
        sb.AppendLine("- `Distill(sessionSummary, wing?)` — scaffold wiki pages from session summary.");
        ...
        sb.AppendLine("- `RecordEpisode(goal, plan, outcome, ..., wing?)` — capture what worked/failed this session.");
        ...
        sb.AppendLine("1. Call `RecordEpisode(goal, plan, outcome, ..., wing?)` — captures what worked/failed.");
        sb.AppendLine("2. Call `Distill(sessionSummary, wing?)` — crystallizes session into wiki pages.");
